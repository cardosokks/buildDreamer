...
export const SettingsPage: React.FC = () => {
  ...
  const handleDownloadN8nWorkflow = () => {
    const codeMapNode = [
      ...
      "    return { json: { ...body, ...page, pageIndex: index + 1, totalPages: body.pages.length, geminiModel: body.geminiModel || '" + geminiModel + "', apiKey: body.apiKey || '" + geminiKey + "' } };",
      "  });",
      "}",
      "// Requisição individual de 1 página por vez:",
      "return [{ json: { ...body, pageIndex: body.pageIndex || 1, totalPages: body.totalPages || 1, geminiModel: body.geminiModel || '" + geminiModel + "', apiKey: body.apiKey || '" + geminiKey + "' } }];"
    ].join("\n");
    ...
    const codeAggregateNode = [
      ...
      "  let parsed = {};",
      "  try {",
      "    const rawText = item.message?.content || item.candidates?.[0]?.content?.parts?.[0]?.text || item.text || item.output || item.response || '{}';",
      ...
      "  } catch (e) {",
      "    parsed = { html: item.html || '<div></div>', css: item.css || '', js: item.js || '' };",
      "  }",
      "  return { json: { explanation: 'Página remasterizada com sucesso via n8n', html: parsed.html || item.html || '<div></div>', css: parsed.css || item.css || '', js: parsed.js || item.js || '' } };",
      "}",
