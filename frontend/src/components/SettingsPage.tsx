...
export const SettingsPage: React.FC = () => {
  ...
  const handleDownloadN8nWorkflow = () => {
    ...
    const codeAggregateNode = [
      ...
      "  let origSlug = inputJson.slug || 'index';",
      "  let origName = inputJson.name || ('Página ' + (index + 1));",
      "  let origHtml = inputJson.html || '<div></div>';",
      "  let origCss = inputJson.css || '';",
      "  let origJs = inputJson.js || '';",
      "  try {",
      "    if ($node['Mapear Estrutura e Links do Site'] && $node['Mapear Estrutura e Links do Site'].all()[index]) {",
      "      const origItem = $node['Mapear Estrutura e Links do Site'].all()[index].json;",
      "      if (origItem.slug) origSlug = origItem.slug;",
      "      if (origItem.name) origName = origItem.name;",
      "      if (origItem.html) origHtml = origItem.html;",
      "      if (origItem.css) origCss = origItem.css;",
      "      if (origItem.js) origJs = origItem.js;",
      "    }",
      "  } catch(e) {}",
      "  let rawText = inputJson.message?.content || inputJson.candidates?.[0]?.content?.parts?.[0]?.text || inputJson.text || inputJson.content || '';",
      "  let extractedHtml = '';",
      "  let extractedCss = '';",
      "  let extractedJs = '';",
      "  if (rawText) {",
      ...
      "          extractedHtml = parsed.html || ''; extractedCss = parsed.css || ''; extractedJs = parsed.js || '';",
      "          parsedSuccess = true;",
      "        }",
      "      } catch (e) {}",
      "    }",
      "    if (!parsedSuccess) {",
      "      const htmlStartIdx = cleanText.indexOf('\"html\"');",
      "      if (htmlStartIdx !== -1) {",
      "        const colonIdx = cleanText.indexOf(':', htmlStartIdx);",
      "        const quoteStart = cleanText.indexOf('\"', colonIdx + 1);",
      "        if (quoteStart !== -1) {",
      "          let rest = cleanText.substring(quoteStart + 1);",
      "          rest = rest.replace(/\\\\\"/g, '\"').replace(/\\\\n/g, '\\n').replace(/\\\\\\\\/g, '\\\\');",
      "          const nextKeyMatch = rest.match(/\"\\s*,\\s*\"(?:css|js|explanation)\"/i);",
      "          if (nextKeyMatch) { extractedHtml = rest.substring(0, nextKeyMatch.index).trim(); }",
      "          else { extractedHtml = rest.replace(/\"\\s*\\}?\\s*$/g, '').trim(); }",
      "        }",
      "      }",
      "      const cssMatch = cleanText.match(/\"css\"\\s*:\\s*\"([\\s\\S]*?)(?:\"\\s*,\\s*\"|\"\\s*\\}|$)/i);",
      "      if (cssMatch && cssMatch[1]) extractedCss = cssMatch[1].replace(/\\\\\"/g, '\"').replace(/\\\\n/g, '\\n');",
      "      const jsMatch = cleanText.match(/\"js\"\\s*:\\s*\"([\\s\\S]*?)(?:\"\\s*,\\s*\"|\"\\s*\\}|$)/i);",
      "      if (jsMatch && jsMatch[1]) extractedJs = jsMatch[1].replace(/\\\\\"/g, '\"').replace(/\\\\n/g, '\\n');",
      "    }",
      "  }",
      "  return { slug: origSlug, name: origName, html: (extractedHtml && extractedHtml.trim().length > 20) ? extractedHtml : origHtml, css: extractedCss || origCss, js: extractedJs || origJs };",
      "});",
