...
function loadBell(): BellNotification[] {
  try { const r = localStorage.getItem(BELL_KEY); return r ? JSON.parse(r) : []; } catch { return []; }
}
function saveBell(items: BellNotification[]) {
  try { localStorage.setItem(BELL_KEY, JSON.stringify(items.slice(0, MAX_BELL))); } catch {}
}
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => { saveBell(bellNotifications); }, [bellNotifications]);
...
otif__;
  ...
  const info = useCallback((m: string, t = 'Informacao', d = 3500) => notify(m, 'info', t, d), [notify]);
  const warning = useCallback((m: string, t = 'Atencao', d = 4000) => notify(m, 'warning', t, d), [notify]);

  const addBellNotification = useCallback((notification: Omit<BellNotification, 'id' | 'read' | 'createdAt'>) => {
    const newItem: BellNotification = { ...notification, id: ell__, read: false, createdAt: new Date().toISOString() };
  ...
  return (
    <NotificationContext.Provider value={{ notify, success, error, info, warning, removeNotification, addBellNotification, markBellRead, markAllBellRead, removeBellNotification, clearBell, bellNotifications, unreadBellCount }}>
      {children}
      <div className="fixed bottom-5 right-5 z-[9999] flex flex-col gap-2.5 max-w-sm w-full pointer-events-none px-3 sm:px-0">
        {notifications.map((item) => {
          const isS = item.type === 'success', isE = item.type === 'error', isW = item.type === 'warning';
          return (
            <div key={item.id} className={pointer-events-auto flex items-start justify-between gap-3 p-3.5 rounded-2xl shadow-2xl backdrop-blur-md border transition-all duration-200 animate-in slide-in-from-bottom-3 fade-in }>
              <div className="flex items-start gap-2.5 min-w-0">
                <div className="mt-0.5 shrink-0">
                  {isS ? <CheckCircle2 className="w-4 h-4 text-emerald-400" /> : isE ? <AlertCircle className="w-4 h-4 text-red-400" /> : isW ? <AlertTriangle className="w-4 h-4 text-amber-400" /> : <Info className="w-4 h-4 text-purple-400" />}
                </div>
              ...
              <button onClick={() => removeNotification(item.id)} className="p-1 -mr-1 -mt-1 opacity-60 hover:opacity-100 transition-opacity rounded-lg hover:bg-white/10 cursor-pointer"><X className="w-3.5 h-3.5" /></button>
            </div>
          );
        })}
      </div>
